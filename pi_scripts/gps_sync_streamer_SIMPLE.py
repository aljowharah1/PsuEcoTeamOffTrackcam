...
# ============== CONFIGURATION ==============
# Camera settings
CAMERA_DEVICE = "/dev/video0"  # USB Global Shutter Camera (FIXED: was /dev/video8)
CAMERA_WIDTH = 1280
CAMERA_HEIGHT = 720
CAMERA_FPS = 30
JPEG_QUALITY = 80
    ...
    try:
        ...
        if result:
            ...
            for pid in pids:
                if pid:
                    try:
                        print(f"[PORT] Killing process {pid} using port {port}")
                        os.kill(int(pid), signal.SIGTERM)
                        time.sleep(0.5)
                    except ProcessLookupError:
                        pass
            print(f"[PORT] Port {port} cleared")
    except Exception as e:
        print(f"[PORT] Error checking port: {e}")
    ...
    try:
        ...
        if result:
            ...
            for pid in pids:
                if pid:
                    try:
                        print(f"[CAM] Killing process {pid} using camera")
                        os.kill(int(pid), signal.SIGTERM)
                        time.sleep(0.5)
                    except ProcessLookupError:
                        pass
            print(f"[CAM] Camera device cleared")
    except Exception as e:
...
def main():
    ...
    # Clear port and camera before starting
    print("\n[INIT] Checking for resource conflicts...")
    kill_port_users(HTTP_PORT)
    kill_camera_users()
